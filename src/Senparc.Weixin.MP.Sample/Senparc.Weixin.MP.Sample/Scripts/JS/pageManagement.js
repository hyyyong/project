...
function pageManagement(pageObj){
	...
	this.leftPaneFunc = null; //左边选项界面事件
...
//设置左边界面事件
pageManagement.prototype.setLeftPaneFunc = function(leftPaneFunc_) {
	if(typeof leftPaneFunc_ == 'function') {
		this.leftPaneFunc = leftPaneFunc_;
	}
}
...
pageManagement.prototype.addPage = function(pageId, tabTitle, url) {
	...
	//触发事件
	if(this.leftPaneFunc != null && typeof this.leftPaneFunc == 'function') {
		this.leftPaneFunc(this.currentShowPage.getPageId());
	}
}
 ...
 * 关闭一个页面
 */
pageManagement.prototype.closePage = function(pageId) {
	if(pageId != null && this.isPageExists(pageId)) {
		var page_ = this.mapManagePage.get(pageId);
		if(page_ != null) {
			this.deletePage(page_);
		}
	}
}
