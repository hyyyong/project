...
(function ($) {
    ...
    $.addFlex = function (t, p) {
        ...
        p = $.extend({ //apply default properties
            ...
            rpOptions: [10, 15, 20, 50, 100, 200, 500],//allowed per-page values//可选择设定的每页结果数  
            title: false, //是否包含标题
            ...
            addData: function (data, type) { //parse data
                ...
                if (p.dataType == 'json') {
                    $.each(data.rows, function (i, row) {
                        ...
                        if ($('thead', this.gDiv).length < 1) {//handle if grid has no headers 无标题
                ...
                if (p.onChangeSort) {
                    ...
                    p.lstTableData.sort(function sortInfo(data1, data2) {
                        ...
                        if (!bnum1 && !bnum2) {
                            ...
                            return data1[paramVals].localeCompare(data2[paramVals]);
                        }
                        //如果参数1为数字，参数2为字符串
                        if (bnum1 && !bnum2) {
                            return -1;
            ...
            showLoading: function (flag) {
                if (flag) {
                    if (!($('.lockmask-flexgrid', 'body') && $('.lockmask-flexgrid', 'body').get(0))) {
                        ...
                        content += '<div class="lockmask-loading-flexgrid">' + parent.lang.loading + '</div>';
                        content += '</div>';
                        content += '</div>';
                        $('body').append(content);
                    }
