    ...
    oTableInit.Init = function () {
        $('#vehicleInfoTable').bootstrapTable({
           ...
           // showRefresh: true,                  //是否显示刷新按钮
            ...
            height: 656,                        //行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
...
function editGroup(vehGroupID) {
    ...
    layer.open({
        ...
        content: ['editGroup.html?&id=' + vehGroupID, 'no'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
    });
...
function addVeh(vehGroupID, e) {
    ...
    layer.open({
        ...
        //offset: '10px',//offset: ['100px', '50px']
        type: 2, //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
        content: ['addVeh.html?&id=' + vehGroupID + '&vehGroupName=' + escape(vehGroupName)] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
    });
...
function AddGroup(theUpdateGroupID, groupName) {
    ...
    layer.open({
        ...
        //offset: '10px',//offset: ['100px', '50px']
        type: 2, //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
        content: ['addGroup.html?&id=' + theUpdateGroupID + '&groupName=' + escape(groupName), 'no'],//这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
    });
