...
$(function () {
    ...
    //getUser(id);
    var m_accountType = getQueryString("accountType");

    if (m_accountType == 1 || m_accountType == 0) {
...
function onSureClick() {
    ...
    $.ajax({
        ...
        //  async: true,                                //是否异步
        data: { "userId": User.userId, "token": User.token, 'userName': $("#userName").val(), 'passWord': $("#passWord").val(), 'code': $("#code").val(), 'memo': $("#memo").val(), 'ownerName': $("#ownerName").val(), 'phone': $("#phone").val(), 'accountType': $("#accountType").val(), 'fGroupID': fGroupID, 'vehGroupID': "-1", 'funcID': "1,2" },
        beforeSend: function () {
            //alert('请求之前');
        },
