...
//是否有视频信号丢失报警 （视频部标）
standardStatus.prototype.isVideoLostFlag = function(){
	if (this.status2 !== null) {
		var valid = ((this.status2 >> 0)&0x01);
		if (valid == 1) {
			return true;
		}
	}
...
//视频信号丢失报警状态 （视频部标）
standardStatus.prototype.getVideoLostFlag = function(device) {
	var ret = {};
	var alarm = []; //list
	var normal = [];
	if(this.isExtraFlagVideo() && this.isVideoLostFlag()) {
		if(this.sensor1){
			for(var i = 0 ; i < 32 ; i ++){
				if((this.sensor1 >> i)&0x01 == 1){
					if(this.isAlarmShield('240,290')) {
						normal.push(device.getSingleChnName(i));
					}else {
						alarm.push(device.getSingleChnName(i));//通道号放进去
					}
				}
			}
		}
	...
	var nomalInfos = '';
	if(alarm.length > 0){
		alarmInfos =parent.lang.video_lost +":  "+ alarm.toString();
	}
	if(normal.length > 0){
		nomalInfos = parent.lang.video_lost+":  " + normal.toString();
	}
	ret.normal = nomalInfos;
	ret.alarm = alarmInfos;
	return ret;
}

//是否有视频信号遮挡报警  视频部标）	
standardStatus.prototype.isVideoMaskFlag = function(){
	if (this.status2 !== null) {
		var valid = ((this.status2 >> 1)&0x01);
		if (valid == 1) {
			return true;
		}
	}
...
//视频信号遮挡报警状态  视频部标）	
standardStatus.prototype.getVideoMaskFlag = function(device) {
	var ret = {};
	var alarm = []; //list
	var normal = []; //list
	if(this.isExtraFlagVideo() && this.isVideoMaskFlag()) {
		if(this.sensor2){
			for(var i = 0 ; i < 32 ; i ++){
				if((this.sensor2 >> i)&0x01 == 1){
					if((this.sensor1 >> i)&0x01 == 1){
						if(this.isAlarmShield('241,291')) {
							normal.push(device.getSingleChnName(i));
						}else {
							alarm.push(device.getSingleChnName(i));//通道号放进去
						}
	...
	var normalInfos = '';
	if(alarm.length > 0){
		alarmInfos =  parent.lang.video_mask+":  "+alarm.toString();
	}
	if(normal.length > 0){
		normalInfos =  parent.lang.video_mask+":  "+ normal.toString();
	}
	ret.normal = normalInfos;
	ret.alarm = alarmInfos;
	return ret;
}

//是否有存储单元故障报警 （视频部标）
standardStatus.prototype.isDiskErrFlag = function(){
	if (this.status2 !== null) {
		var valid = ((this.status2 >> 2)&0x01);
		if (valid == 1) {
			return true;
		}
	}
...
//存储单元故障报警状态 （视频部标）
standardStatus.prototype.getDiskErrFlag = function() {
	var ret = {};
	var zc = [];//主存
	var bz = [];//备灾
	if(this.isExtraFlagVideo() && this.isDiskErrFlag()) {
		if(this.sensor3){
			for(var i = 0 ; i < 16 ; i ++){
				if((this.sensor3 >> i)&0x01 == 1){
					if(i < 12 ){
						zc.push((i+1));//通道号放进去
					}else{
						bz.push((i+1));//通道号放进去
	...
	var result = '';
	if(zc.length > 0){
		result += zc.join(',')+ parent.lang.disk_primary_storage;
	}
	if(bz.length > 0){
		result += bz.join(',')+  parent.lang.disk_disaster_recovery;
	}
	if(this.isAlarmShield('242,292')) {
		ret.normal = result;
		ret.alarm = '';
	}else {
		ret.normal = '';
...
standardStatus.prototype.getOtherDiskErrFlag = function() {
	...
	if(this.isExtraFlagVideo() && this.isOtherDiskErrFlag()) {
		infos = parent.lang.other_device_error;
	}
	if(this.isAlarmShield('243,293')) {
		ret.normal = infos;
		ret.alarm = '';
	}else {
		ret.normal = '';
...
//是否有异常驾驶行为报警 （视频部标）
standardStatus.prototype.isAbnormalDriveFlag = function(){
	if (this.status2 !== null) {
		var valid = ((this.status2 >> 4)&0x01);
		if (valid == 1) {
			return true;
		}
	}
...
//异常驾驶行为报警状态 （视频部标）
standardStatus.prototype.getAbnormalDriveFlag = function() {
	var infos = [];
	var ret = {};
	if(this.isExtraFlagVideo() && this.isAbnormalDriveFlag()) {
		if(this.sensor4){
			if((this.sensor4 >> 0)&0x01 == 1){
				var cd = '0';
				if(this.sensor5){
					cd = this.sensor5;
				}
				infos.push(parent.lang.fatigue_driving+"["+cd+"]");
			}
			if((this.sensor4 >> 1)&0x01 == 1){
				infos.push(parent.lang.on_the_telephone);
			}
			if((this.sensor4 >> 2)&0x01 == 1){
				infos.push(parent.lang.smoking);
			}
		}
	}
...
standardStatus.prototype.getRapidAcceleration = function() {
	...
	if(this.isRapidAcceleration()) {
		infos = parent.lang.alarm_type_rapidAcceleration;
	}
	var ret  = {};
...
standardStatus.prototype.getRapidDeceleration = function() {
	...
	if( this.isRapidDeceleration()) {
		infos = parent.lang.alarm_type_rapidDeceleration;
	}
	var ret  = {};
...
standardStatus.prototype.isParking = function() {
	if(this.isStillEvent() && this.isAccOpen()){//} && !this.isParkOverTime()) {
		return true;
	}
	return false;
}
...
standardStatus.prototype.getRunStatus = function() {
	if(this.status2 != null) {
		if(((this.status2>>20)&1 > 0) && this.isJT808Protocol()) {
			...
			return parent.lang.alarm_type_run_status;//运营
		}*/
	}
	return '';
}
...
standardStatus.prototype.getEncipherStatus = function() {
	if(this.status2 != null) {
		if(((this.status2>>21)&1 > 0) && this.isJT808Protocol()) {
			...
			return parent.lang.alarm_type_not_encipher;
		}*/
	}
	return '';
}
...
standardStatus.prototype.getOilStatus = function() {
	if(this.status2 != null) {
		if(((this.status2>>22)&1 > 0) && this.isJT808Protocol()) {
			...
			return parent.lang.alarm_type_oil_normal;
		}*/
	}
	return '';
}
...
standardStatus.prototype.getObdStatus = function() {
	...
	if(this.obdSpeed != null){
		str += parent.lang.report_speed + parent.lang.labelKmPerHour + ":" + this.obdSpeed;
	}else{
...
standardStatus.prototype.getElectricStatus = function() {
	if(this.status2 != null) {
		if(((this.status2>>23)&1 > 0) && this.isJT808Protocol()) {
			...
			return parent.lang.alarm_type_electric_normal;
		}*/
	}
	return '';
}
...
standardStatus.prototype.getDoorStatus = function() {
	if(this.status2 != null) {
		if(((this.status2>>24)&1 > 0) && this.isJT808Protocol()) {
			...
			return parent.lang.alarm_type_unlock;
		}*/
	}
	return '';
}
...
standardStatus.prototype.getVideoLostStatus = function(device){
	...
	var str = '';
	if(alarm.length > 0) {
		str = /*'<span class="b">' + parent.lang.alarm_type_video_lost_status + '：</span>' + */alarm.toString();
	}
...
standardStatus.prototype.getRecordStatus = function(device){
	...
	var str = '';
	if(alarm.length > 0) {
		str = /*'<span class="b">' + parent.lang.alarm_type_record_state + '：</span>' + */alarm.toString();
	}
//	if(this.isAlarmShield('4,54')) {
		ret.normal = str;
//	}else {
	...
	if(this.status4 != null) {
		...
		if((this.status4>>17)&1 > 0) {
			if(this.isAlarmShield('213,263')) {
				normal.push(parent.lang.alarm_type_InadequateOrTooLongRoadTravelTime);
			}else {
				alarm.push(parent.lang.alarm_type_InadequateOrTooLongRoadTravelTime);
			}
		}
...
standardStatus.prototype.getTemperature = function(tempCount, tempName, sensorType, type) {
	if (tempCount != null && tempCount > 0 && tempName != null) {
		...
		if(type == 1 || type == 2){//氧气
			...
			str = parent.lang.alarm_temperator_unit;
		}
...
//		}
	...
	//第0位	视频信号丢失报警 （视频部标）
	var videoLostFlag = this.getVideoLostFlag(device);
	if(videoLostFlag.normal != '') {
		normal.push(videoLostFlag.normal);
	}
	if(videoLostFlag.alarm != '') {
		alarm.push(videoLostFlag.alarm);
	}
	//第1位	视频信号遮挡报警  视频部标）
	var videoMaskFlag = this.getVideoMaskFlag(device);
	if(videoMaskFlag.normal != '') {
		normal.push(videoMaskFlag.normal);
	}
	if(videoMaskFlag.alarm != '') {
		alarm.push(videoMaskFlag.alarm);
	}
	//存储单元故障报警 （视频部标）
	var diskErrFlag = this.getDiskErrFlag();
	if(diskErrFlag.normal != '') {
		normal.push(diskErrFlag.normal);
	}
	if(diskErrFlag.alarm != '') {
		alarm.push(diskErrFlag.alarm);
	}
	//其他视频设备故障报警（视频部标）
...
standardStatus.prototype.parseStatusInfo = function() {
	...
	if(vehicle.isPeopleTerminal()) {
		//人员名称
		html.push('<span class="b">'+ parent.lang.track_labelName +'</span>'+ vehicle.getPeopleName() +'<br/>');
		//电话
		html.push('<span class="b">'+ parent.lang.track_labelTelPhone +'</span>'+ vehicle.getTelePhone() +'<br/>');
		//时间
		...
		html.push('<span class="b">' + parent.lang.labelTime + '</span>' + ret.gpsTime + '<br/>');
		//里程
		html.push('<span class="b">' + parent.lang.monitor_labelLiCheng + '</span>' + ret.liCheng + '<br/>');
		//速度
	...
	var abnormalDriveFlag_ = this.getAbnormalDriveFlag();
	...
	if(abnormalDriveFlag_.normal != '') {
		normal.push(abnormalDriveFlag_.normal);
	}
	if(abnormalDriveFlag_.alarm != '') {
		alarm.push(abnormalDriveFlag_.alarm);
	}
	...
	if(device.isOilSensorSupport()) {
		...
		//油量信息
		html.push('<span class="b">' + parent.lang.labelFuel + '</span>&nbsp;' + this.getYouLiangStr() + '<br/>');
	}
	...
	if(!vehicle.isPeopleTerminal()) {
		//正常状态
		html.push('<span class="b">' + parent.lang.monitor_labelNormal + '</span>' + normalString + '<br/>');
		//有报警才显示
		if(alarmString != '') {
			//报警
			html.push('<span class="b red">' + parent.lang.monitor_labelAlarm + '</span>&nbsp;<span class="red">' + alarmString + '</span><br/>');
		}
		//录像状态
		var videoStatus = "";
		if(recordStatus.length > 0) {
			videoStatus += '<span class="b">' + parent.lang.monitor_video_status + '</span>&nbsp;' + recordStatus.toString();
		}
	...
	if(recordStatus.length > 0) {
		...
		ret.normal += parent.lang.monitor_video_status + recordStatus.toString();
	}
	if(videoLostStatus.length > 0) {
		if(ret.normal != '') {
			ret.normal += ",";
		}
		ret.normal += parent.lang.alarm_type_video_lost_status + '：' + videoLostStatus.toString();
	}
	if(IOStatus.length > 0) {
		if(ret.normal != '') {
			ret.normal += ",";
		}
		ret.normal += parent.lang.alarm_type_io_high + '：' + IOStatus.toString();
	}
