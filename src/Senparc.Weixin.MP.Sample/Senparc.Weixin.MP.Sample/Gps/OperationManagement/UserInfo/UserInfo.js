...
function editUser(userId) {
    ...
    layer.open({
        ...
        content: ['EditUser/EditUser.html?&id=' + userId, 'no'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
    });
...
function bindVehGroup(parentId, userId) {
    ...
    layer.open({
        ...
        offset: '10px',//offset: ['100px', '50px']
        type: 2, //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
        content: ['./AddUser/bindVehGroup.html?&userId=' + userId, 'no'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
    });
}
...
function addChildUser(userId, e) {
    layer.open({
        ...
        offset: '10px',//offset: ['100px', '50px']
        type: 2, //0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
        content: ['AddUser/AddUser.html?&id=' + userId + "&name=" +escape( $(e).attr("name")), 'no'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
    }); 
   // AddUser(userId);
