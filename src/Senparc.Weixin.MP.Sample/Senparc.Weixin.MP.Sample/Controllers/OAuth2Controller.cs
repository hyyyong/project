...
namespace Senparc.Weixin.MP.Sample.Controllers
{
    public class OAuth2Controller : Controller
    {
        ...
        public ActionResult Login(string code, string state)
        {
            ...
            if (result.errcode != ReturnCode.请求成功)
            {
                return Content("错误：" + result.errmsg + "===" + appId + "===" + secret + "===" + result.errcode);
            }
            //下面2个数据也可以自己封装成一个类，储存在数据库中（建议结合缓存）
            //如果可以确保安全，可以将access_token存入用户的cookie中，每一个人的access_token是不一样的
            Session["OAuthAccessTokenStartTime"] = DateTime.Now;
            Session["OAuthAccessToken"] = result;
            //result.openid 用户微信id
            //List<UserInfo> usreInfoList = new List<UserInfo>();
            //if (!M_Login.GetUserList(result.openid.ToString(), out usreInfoList))
            //{
            //    //返回错误
            //}
            // http://wx.car900.com/weixin/indexlogin?code=011NXV5f16oORt0S2f6f1vdQ5f1NXV5P&state=1    

            return RedirectToAction("IndexLogin", "OAuth2", new { wxid = result.openid.ToString() });
        }

        //登录显示微信ID绑定车辆数据数据
        [ActionName("IndexLogin")]
        public ActionResult IndexLogin(string wxid)
        {    
            ViewData["wxid"] = wxid;           
            return View();
        }
